...
if (!isset($_SESSION['userId'])) {
    ...
    if (isset($vorigeBieder)) {
        ...
        if ($melding = $dbh->prepare($sql3)) {
            $melding->execute(array('U bent overboden op  <a href= "productPage.php?ID=' . $_SESSION['PID'] . '">deze veiling</a>. Het huidige bod betreft ' . $valuta . $bod .   '', $vorigeBieder));
        }
    }
    ...
    $sql2 = "SELECT * FROM Voorwerp WHERE VoorwerpNummer = ?";
    ...
    if ($verkoper == $gebruiker) {
        header("Location: $_SERVER[HTTP_REFERER]&error=nietOpEigenVeiling");
        exit();
    }
